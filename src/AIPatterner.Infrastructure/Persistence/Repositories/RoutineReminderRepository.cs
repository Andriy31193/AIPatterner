...
public class RoutineReminderRepository : IRoutineReminderRepository
{
    ...
    public async Task<RoutineReminder?> GetByRoutineAndActionAsync(Guid routineId, string actionType, CancellationToken cancellationToken)
    {
        return await _context.RoutineReminders
            .FirstOrDefaultAsync(r => r.RoutineId == routineId && r.SuggestedAction == actionType, cancellationToken);
    }
